...
constc = require("../models/Medicine_Disease");
...
exports.submitdata = async (req, res, next) => {
  try {
    ...
    const encryptedregistration = encryptCustomId(registration);
...
exports.login = async (req, res, next) => {
  try {
    ...
    const decryptedUser = {
      ...
      registration: decryptCustomId(user.registration),
...
exports.updatethemedicineTbel = async (req, res, next) => {
  try {
    const { encryptedCustomId, updateData } = req.body;

    // Validate if required fields are present in the request body
    if (!encryptedCustomId || !updateData) {
      return res.status(400).json({
        success: false,
        message: "Both encryptedCustomId and updatedData are required fields.",
      });
    }

    // Hash the encryptedCustomId to match with the super_encryptedCustomId in the database
    const hashedSuperEncryptedCustom = crypto
      .createHash("sha256")
      .update(encryptedCustomId)
      .digest("hex");

    // Find the medicine table data for the user
    const medicinetabeldata = await Medicine_Disease.find({
      super_encryptedCustomId: hashedSuperEncryptedCustom,
    });

    // Handle case when no data is found for the specified encryptedCustomId
    if (!medicinetabeldata || medicinetabeldata.length === 0) {
      return res.status(404).json({
        success: false,
        message: "No medicine table data found for the specified user.",
      });
    }
    let updatedData = req.body;

    // Update the medicine table data

    const updatedMedicineData = await Medicine_Disease.findOneAndUpdate(
      { super_encryptedCustomId: hashedSuperEncryptedCustom },
      { $set: updatedData },
      { new: true }
    );
    ...
    if (!updatedMedicineData) {
      return res.status(404).json({
        success: false,
        message: "Failed to update medicine table data.",
      });
    }
    await updatedMedicineData.save();

    res.status(200).json({
      success: true,
      message: "Medicine table data updated successfully",
      updatedMedicineData,
    });
  } catch (error) {
    console.error(error);
